...
%             data_name       = [ '"' data_name '"' ];
        ...
        function obj = addmaterial(obj, materialtype, matname, matprops)
            % Adds a custom material to the material database
            % see
            % https://optics.ansys.com/hc/en-us/articles/360034930013-addmaterial-Script-command
            % for more
            % currently only supports basic isotropic non dispersive
            % materials
            %
            % Args:
            %   materialtype
            %       type: string
            %       desc: one of the options specified when calling (questionmark)material; from lumerical
            %   matname
            %       type: string
            %       desc: name of the new material to use
            %   matprops
            %       type: struct
            %       desc: properties to set on the new material
            
            % init the new material
            obj = obj.write_command( sprintf('mymat = addmaterial("%s");', materialtype) ); 

            % set properties
            obj = obj.write_command( sprintf('setmaterial(mymat,"name","%s");', matname) ); 

            props_to_set = fieldnames( matprops );
            for ii = 1:length(props_to_set)
                % set properties
                prop_name = strrep(props_to_set{ii}, '_', ' ');
                prop_val = matprops.(props_to_set{ii});
                if ischar(prop_val)
                    % property value is a string
